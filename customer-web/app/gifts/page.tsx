...
export default function FoodPage() {
  ...
  useEffect(() => {
    fetchMarketplace('GIFTS').then(m => { setMerchants(m); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="animate-fade-in pb-24">
      ...
      <div className="p-4">
        {loading && <p className="text-center text-zana-muted text-sm py-10">Loading gift shops…</p>}
        {!loading && merchants.length === 0 && (
          <div className="text-center py-16">
            <p className="text-sm text-zana-muted">No gifts merchants available yet.</p>
        ...
        {merchants.map(m => (
          <div key={m.id} className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-10 h-10 rounded-xl bg-zana-primary-light flex items-center justify-center text-lg">🎁</div>
              <div>
