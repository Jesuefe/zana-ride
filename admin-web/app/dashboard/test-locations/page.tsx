  ...
  return <AdminShell><div className="h-[calc(100vh-40px)] min-h-[720px] flex flex-col gap-3">
    ...
    <div className="grid lg:grid-cols-[1fr_370px] gap-3 flex-1 min-h-0">
      ...
      <div className="bg-white rounded-2xl border overflow-auto p-4 space-y-4">
        ...
        {job&&<div className="bg-gray-50 rounded-xl p-3"><p className="text-[10px] uppercase font-black text-gray-400">{activeDelivery?'Delivery':'Ride'} · {job.status}</p><p className="text-xs font-bold mt-1">{activeDelivery?job.itemDescription:(job.customer?.firstName||'Passenger')}</p><p className="text-[11px] text-gray-600 mt-1">{job.pickupAddress||'Pickup'} → {job.dropoffAddress||job.destinationAddress||'Destination'}</p>{activeDelivery&&<><p className="text-[11px] mt-2">Pickup: <b>{job.pickupContactName||'Pickup contact'}</b> · {job.pickupPhone||'—'}</p><p className="text-[11px]">Recipient: <b>{job.recipientName||'Recipient'}</b> · {job.recipientPhone||'—'}</p><div className="grid grid-cols-2 gap-2 mt-3"><button onClick={()=>{if(job.pickup?.lat!=null)moveTo({lat:Number(job.pickup.lat),lng:Number(job.pickup.lng)}).then(()=>setStatus('Driver moved to pickup'))}} className="py-2 rounded-lg bg-amber-100 text-amber-800 text-[10px] font-black">Jump to pickup</button><button onClick={()=>{if(job.dropoff?.lat!=null)moveTo({lat:Number(job.dropoff.lat),lng:Number(job.dropoff.lng)}).then(()=>setStatus('Driver moved to drop-off'))}} className="py-2 rounded-lg bg-zana-primary/10 text-zana-primary text-[10px] font-black">Jump to drop-off</button></div></>}{activeRide?.customer?.phone&&<p className="text-[11px] mt-2">Passenger: {activeRide.customer.phone}</p>}{target&&<p className="text-[11px] mt-2 font-bold text-zana-primary">Target {target.lat.toFixed(5)}, {target.lng.toFixed(5)}</p>}</div>}
