...
export class CallsService {
  ...
  async markConnected(callId: string, userId: string) {
    ...
    if (![CallStatus.ACCEPTED, CallStatus.CONNECTING, CallStatus.CONNECTED].includes(call.status)) {
      throw new BadRequestException(`CALL_NOT_ACTIVE (status: ${call.status})`);
    }
  ...
  async getCallToken(callId: string, userId: string) {
    ...
    if (![CallStatus.RINGING, CallStatus.ACCEPTED, CallStatus.CONNECTING, CallStatus.CONNECTED].includes(call.status)) throw new BadRequestException(`CALL_NOT_ACTIVE (status: ${call.status})`);
