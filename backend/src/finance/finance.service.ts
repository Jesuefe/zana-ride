...
export class FinanceService {
  ...
  async agentSummary(userId: string) {
    ...
    return { agentId: agent.id, marketId: agent.marketId, settlements, totalEarning: settlements.reduce((s,x)=>s+x.agentEarning,0), totalMarkup: settlements.reduce((s,x)=>s+(x.markupAmount ?? x.grossMargin ?? 0),0) };
  }
  ...
  async adminAgent(agentId: string) {
    ...
    return { settlements, markup: settlements.reduce((s,x)=>s+(x.markupAmount ?? x.grossMargin ?? 0),0), agentEarning: settlements.reduce((s,x)=>s+x.agentEarning,0), zanaEarning: settlements.reduce((s,x)=>s+x.zanaEarning,0) };
  }
