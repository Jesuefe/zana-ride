...
const deliveryStatuses = ['REQUESTED','COURIER_ASSIGNED','PICKED_UP','DELIVERED'];
...
function OrderCard({order,fund,onOpen,onUnavailable,onStatus,onWithdraw,busy,lang}:any){
 ...
 const lat=order.delivery?.dropoffLat??order.dropoffLat??order.lat;
 const lng=order.delivery?.dropoffLng??order.dropoffLng??order.lng;
 const hasLocation=validCoordinate(lat)&&validCoordinate(lng);
 const maps=hasLocation?'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(String(lat)+','+String(lng)):null;
...
function DeliveryCard({delivery,onCall}:any){
 ...
 return <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
  <div className="flex justify-between gap-3">
   <div><p className="font-mono text-xs font-bold text-zana-primary">{delivery.order?.trackingCode||delivery.trackingCode}</p><p className="font-bold text-sm">{delivery.order?.customer?.firstName||'Customer'}</p><p className="text-xs text-gray-500">{delivery.dropoffAddress||'Customer location'}</p></div>
   <span className="h-fit text-[10px] font-bold uppercase bg-gray-100 px-2.5 py-1 rounded-full">{stateText}</span>
  </div>
