      ...
      <div className="p-4 space-y-4">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{dt("Deliveries")}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="font-semibold text-gray-900">{dt("Withdraw to Mobile Money")}</p>
          </div>

          {withdrawDone ? (
            <div className="flex items-center gap-2 text-green-600 bg-green-50 rounded-lg px-4 py-3">
              <CheckCircle size={18} />
              <p className="text-sm font-semibold">{dt("Withdrawal sent! Check your phone.")}</p>
            </div>
          ) : (
            <>
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">{dt("Amount (RWF)")}</label>
                  <input
                ...
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">{dt("MTN / Airtel number")}</label>
      ...
      {showSettle && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/50" onClick={closeSettleModal}>
          <div className="w-full bg-white rounded-t-3xl p-5 pb-8" onClick={e => e.stopPropagation()}>
            ...
            {settleStage === 'form' && (
              <>
                <p className="font-black text-lg text-gray-900 mb-1">{dt("Pay Zana")}</p>
            ...
            {settleStage === 'waiting' && (
              <div className="flex flex-col items-center py-6">
                ...
                <p className="font-semibold text-gray-900">{dt("Check your phone")}</p>
                <p className="text-sm text-gray-500 mt-1 text-center">{dt("Approve the MoMo prompt to complete payment")}</p>
              </div>
            )}
            ...
            {settleStage === 'success' && (
              <div className="flex flex-col items-center py-6">
                ...
                <p className="font-semibold text-gray-900">{dt("Payment received")}</p>
                <p className="text-sm text-gray-500 mt-1">{dt("Your Zana due balance is now settled")}</p>
                <button onClick={closeSettleModal} className="mt-5 text-sm font-semibold text-zana-primary">{dt("Done")}</button>
              </div>
            )}
            ...
            {settleStage === 'failed' && (
              <div className="flex flex-col items-center py-6">
                ...
                <p className="font-semibold text-gray-900">{dt("Payment didn't go through")}</p>
                <button
